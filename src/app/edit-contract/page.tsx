...
const EditContract = () => {
  ...
  const handleNextClick = () => {
    setLoading(true);
    setTimeout(() => {
      router.push('/edit-contract/edit-location');
    }, 1000); // simulate delay
  };
    ...
    <Box sx={{ display: 'flex', gap: 15, alignItems: 'flex-start', flexWrap: 'wrap' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, maxWidth: 400 }}>
        <Box sx={{ display: 'flex', gap: 2 }}>
          ...
          <Typography sx={{ fontWeight: 600, fontSize: 18 }}>Edit contract</Typography>
        </Box>

        <Typography variant="h6" sx={{ fontWeight: 600 }}>Contract details</Typography>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minWidth: 300 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
          <label style={{ fontSize: 14, fontWeight: 500, color: '#1A1A1A' }}>Job title</label>
          <TextField
            fullWidth
